...
async function ollamaChat(message, training = false) {
  ...
  const r = await fetch(`${ollamaUrl}/api/chat`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: ollamaModel, stream: false, messages: [{ role: 'system', content: system }, { role: 'user', content: message }], options: { temperature: 0.35 } }) });
...
async function openRouterChat(message, training = false) {
  ...
  const system = `${systemPrompt} ${training ? 'You are in TRAINING MODE: teach clearly with concepts, examples and practice.' : ''}`;
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', { method: 'POST', headers: { Authorization: `Bearer ${key}`, 'Content-Type': 'application/json', 'HTTP-Referer': 'http://localhost:3000', 'X-Title': 'Prometheus AI OS' }, body: JSON.stringify({ model: openRouterModel, stream: false, messages: [{ role: 'system', content: system }, { role: 'user', content: message }], temperature: 0.35, max_tokens: 500 }) });
...
async function getAI(message, training) {
  if (currentMode === 'offline') return { reply: await ollamaChat(message, training), provider: 'LOCAL', model: ollamaModel };
  if (currentMode === 'online') return { reply: await openRouterChat(message, training), provider: 'ONLINE', model: openRouterModel };
  try { return { reply: await ollamaChat(message, training), provider: 'LOCAL', model: ollamaModel }; }
  catch (localError) { try { return { reply: await openRouterChat(message, training), provider: 'ONLINE', model: openRouterModel }; } catch { throw new Error(`No AI provider available. Start Ollama for offline mode or configure OpenRouter for online mode. Local: ${localError.message}`); } }
}

app.get('/api/status', async (_req, res) => { let local = false; try { const r = await fetch(`${ollamaUrl}/api/tags`); local = r.ok; } catch {} res.json({ mode: currentMode, local, online: Boolean(process.env.OPENROUTER_API_KEY), model: local ? ollamaModel : openRouterModel }); });
